...
export default function ProfileClient({ profile, recentScores, totalPoints, totalBacs, userId }: Props) {
  ...
  // Cet état conserve le profil pour l'affichage et est mis à jour de manière optimiste.
  const [currentProfile, setCurrentProfile] = useState(profile)

  // Ces états sont uniquement pour les champs du formulaire.
  const [username, setUsername] = useState(currentProfile?.username ?? '')
  const [selectedEmoji, setSelectedEmoji] = useState(currentProfile?.avatar_url ?? '🦊')
  const [editing, setEditing] = useState(false)
  const [saving, setSaving] = useState(false)

  // Garde l'état local synchronisé si les props du serveur changent
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    if (result?.error) {
      ...
      // Met à jour l'interface de manière optimiste
      setCurrentProfile((p: any) => ({ ...p, username: trimmedUsername, avatar_url: selectedEmoji }))
      setEditing(false)
      // Rafraîchit les données du serveur pour assurer la cohérence
      router.refresh()
    }
  }
  ...
  // Grouper les scores par partie (room code)
  const games = useMemo(() => {
    ...
    for (const s of recentScores) {
      ...
      if (!gameMap.has(code)) {
        gameMap.set(code, {
          code,
          rounds: s.rounds?.rooms?.round_count ?? 0,
          pts: 0,
          letter: s.rounds?.letter ?? '?'
        })
      }
          ...
          {editing ? (
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-6 gap-2">
                  {AVATARS.map((emoji) => (
                    <button
                      ...
                      className={`text-3xl h-12 rounded-xl transition-all ${
                        selectedEmoji === emoji
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    setEditing(false) // Les états du formulaire seront réinitialisés à la prochaine ouverture via le bouton "Modifier"
                  }}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-zinc-500 text-sm">Joueur Petit Bac</p>
              </div>
              <button
                onClick={() => {
                  setUsername(currentProfile?.username ?? '')
                  setSelectedEmoji(currentProfile?.avatar_url ?? '🦊')
                  setEditing(true)
                }}
              ...
              {games.map((game, i) => (
                <div key={game.code} className="flex items-center justify-between py-2 border-b border-zinc-800 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="w-8 h-8 bg-zinc-800 rounded-lg flex items-center justify-center text-yellow-400 font-black text-sm">
                      {game.letter}
                    </div>
                    <div>
